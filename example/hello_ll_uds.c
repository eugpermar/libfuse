...
static VduseDev *create_vdpa(void *priv) {
	...
	fprintf(stderr, "[eperezma %s:%d][vduse_dev_create enter]\n", __func__, __LINE__);
	vduse_dev = vduse_dev_create(vduse_name, device_id, vendor_id,
	                             features, num_queues, config_size, config,
	                             &vduse_ops, priv);
	fprintf(stderr, "[eperezma %s:%d][vduse_dev_create exit]\n", __func__, __LINE__);
	assert(vduse_dev);
	...
	fprintf(stderr, "[eperezma %s:%d][netlink_add_vduse enter]\n", __func__, __LINE__);
	r = netlink_add_vduse("fsd", VDPA_CMD_DEV_NEW);
	fprintf(stderr, "[eperezma %s:%d][netlink_add_vduse exit]\n", __func__, __LINE__);
	assert (r == 0);
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		fprintf(stderr, "[eperezma %s:%d][poll_one_forever devfd ret=%d][errno=%d]", __func__, __LINE__, ret, errno);
		assert(ret > 0);
